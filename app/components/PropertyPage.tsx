...
/* ─── AMENITY LABEL MAP (snake_case to display names) ─── */
const amenityLabelMap: Record<string, { es: string, en: string }> = {
  'infinity_pool': { es: 'PISCINA INFINITA', en: 'INFINITY POOL' },
  'private_gym': { es: 'GIMNASIO PRIVADO', en: 'PRIVATE GYM' },
  'indoor_jacuzzi': { es: 'JACUZZI INTERIOR', en: 'INDOOR JACUZZI' },
  'garden': { es: 'JARDÍN EXTERIOR', en: 'OUTDOOR GARDEN' },
  'private_terrace': { es: 'TERRAZA PRIVADA', en: 'PRIVATE TERRACE' },
  'panoramic_view': { es: 'VISTA PANORÁMICA', en: 'PANORAMIC VIEW' },
  'gourmet_kitchen': { es: 'COCINA GOURMET', en: 'GOURMET KITCHEN' },
  'parking': { es: 'ESTACIONAMIENTO', en: 'PARKING' },
  'security_24h': { es: 'SEGURIDAD 24H', en: 'SECURITY 24H' },
}


const amenityIcons: Record<string, React.ReactNode> = {
  'infinity_pool': <svg width="36" height="36" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}><path d="M3 16h18M3 16a9 9 0 0118 0M6 16V10a6 6 0 0112 0v6"/><path d="M2 20h20M12 4v2"/></svg>,
  'private_gym': <svg width="36" height="36" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}><path d="M3 10h3v8H3zM10 2v16M14 2v16M21 10h-3v8h3"/><path d="M10 2l2-1 2 1M14 2l2-1 2 1M10 18l2 1 2-1M14 18l2 1 2-1"/></svg>,
  'indoor_jacuzzi': <svg width="36" height="36" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}><circle cx="6" cy="16" r="1.5"/><circle cx="12" cy="16" r="1.5"/><circle cx="18" cy="16" r="1.5"/><path d="M2 12c0-5 4-8 10-8s10 3 10 8v3H2z"/><path d="M12 4v3M2 15h20"/></svg>,
  'garden': <svg width="36" height="36" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}><path d="M12 2v3M8 8a4 4 0 018 0M5 13a6 6 0 0114 0M2 20h20M8 13l2-3 2 3M14 13l2-3 2 3"/></svg>,
  'private_terrace': <svg width="36" height="36" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}><path d="M2 6h20v12H2zM2 6L4 2h16l2 4M8 6v12M16 6v12M2 18h20"/></svg>,
  'panoramic_view': <svg width="36" height="36" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}><path d="M1 14l3-4 4 5 3-4 4 6 5-7v10H1z"/><circle cx="6" cy="8" r="1.5"/></svg>,
  'gourmet_kitchen': <svg width="36" height="36" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}><path d="M3 9h18M5 3v16M19 3v16M8 3h1v2M12 3h1v2M16 3h1v2"/><rect x="7" y="9" width="10" height="10" rx="1"/></svg>,
  'parking': <svg width="36" height="36" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M8 8h8v8H8z"/><path d="M9 12h6M9 15h4"/><path d="M14 8v8"/></svg>,
  'security_24h': <svg width="36" height="36" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.2}><path d="M12 2l8 3v5c0 7-8 8-8 8s-8-1-8-8V5l8-3z"/><path d="M10 14l2 2 4-5"/></svg>,
}
                  ...
                  <div key={g.key} className={styles.amenitiesCategoria}>
                    ...
                    <div className={styles.amenitiesIcons}>
                      {g.items.map((key: string, i: number) => {
                        const label = amenityLabel(key, lang) || amenityLabelMap[key]?.[lang] || key
                        return (
                          <div key={i} className={styles.amenityItem}>
                            <div className={styles.amenityIcon}>{amenityIcons[key] || defaultIcon}</div>
